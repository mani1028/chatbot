...
class ChaosTestContext:
    ...
    def __init__(self):
        ...
        with self.app.app_context():
            ...
            # Create test plan first (with unique name to avoid conflicts)
            plan_name = f'TestPlan{uuid.uuid4().hex[:8]}'
            test_plan = Plan(name=plan_name, max_monthly_chats=1000)
            db.session.add(test_plan)
            db.session.commit()
            
            # Create test site with proper relationships (with unique domain)
    ...
    def cleanup(self):
        ...
        try:
            db.session.remove()
            # Don't drop all - just close the session
            # db.drop_all() causes issues with foreign key constraints
        except:
            pass
...
def test_concurrency_100_simultaneous():
    ...
    try:
        ...
        print(f"\n[PASS] TEST 1: Concurrency resilient")
        return True
...
def test_llm_failure_graceful_degradation():
    ...
    try:
        ...
        print(f"\n[PASS] TEST 2: LLM failure handled gracefully")
        return True
...
def test_db_commit_rollback_consistency():
    ...
    try:
        ...
        print(f"\n[PASS] TEST 3: DB failure handled with rollback consistency")
        return True
...
def test_multi_tenant_isolation():
    ...
    try:
        ...
        # Get or create a plan (may already exist from this test context)
        test_plan = Plan.query.filter_by(name=ctx.test_site.plan.name).first() if ctx.test_site.plan else None
        if not test_plan:
        ...
        site_b = Site(
            name='Test Site B',
            domain=f'testb-{uuid.uuid4().hex[:8]}.com',
            status='active',
            plan_id=test_plan.id
        )
        ...
        print(f"\n[PASS] TEST 4: Multi-tenant isolation enforced")
        return True
...
================================================================================
    ...
    if passed == total:
        print("\n[SUCCESS] SYSTEM PRODUCTION-READY: All resilience tests passed")
        print("   Architecture enforced + Chaos tested = Safe for production")
        sys.exit(0)
    else:
        print(f"\n[WARNING] System resilience incomplete: {total-passed} failures")
        sys.exit(1)
