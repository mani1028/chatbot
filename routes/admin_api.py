...
def get_site_template_data(site_id):
    try:
        ...
        intents = Intent.query.filter_by(site_id=site_id).all()
        intent_list = [{'id': i.id, 'name': i.intent_name, 'response': i.response[:50] + '...' if i.response else ""} for i in intents]
        return jsonify({'success': True, 'config': config_dict, 'intent_count': len(intents), 'intents': intent_list})
